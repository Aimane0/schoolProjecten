...
# Titel en search bar
st.title('Check user information through their usernames!')
search_bar = st.text_input('Fill in any username to see their info!')
...
# Als de searchbar ingevuld wordt moet de webpage veranderen en de volgende code uitvoeren
if search_bar:
    # De functies die gemaakt zijn oproepen om nodige info te verkrijgen
    user_id = get_user_id(search_bar)
    ...
    # Als een user niet bestaat een melding geven
    if user_information == {}:
        st.markdown('This user does not exist!')
    else:
        ...
        with st.expander(f'{search_bar}\'s last played games within the last 2 weeks'):
            st.markdown(last_played_games, unsafe_allow_html=True)

        # Expander om de games die de owner heeft te tonen
        with st.expander(f'{search_bar}\'s owned games'):
            st.markdown(owned_games, unsafe_allow_html=True)

        # Tonen van de achievements van games die de user heeft
        with st.expander(f'{search_bar}\'s achievements'):
            owned_games_list.sort()
            default_game = owned_games_list[0]
            achievement_search_bar = st.selectbox(f'Search for a video game by name to see {search_bar}\'s achievements:'
                                        ,owned_games_list, index=owned_games_list.index(default_game))
            if achievement_search_bar:
                achievements = get_achievements(user_id, achievement_search_bar)
                st.markdown(f'<b>{search_bar} has achieved the following achievements in {achievement_search_bar}:</b><br><br>{achievements}', unsafe_allow_html=True)

        # Tonen van de vriendelijst van de gebruiker en de basis informatie van die vrienden
        with st.expander(f'{search_bar}\'s friends, total amount of {len(friendlist_ids)}'):
